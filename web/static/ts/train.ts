...
export async function startTraining(): Promise<void> {
    const epochsEl = document.getElementById('epochs') as HTMLInputElement | null;
    const progressSection = document.getElementById('progressSection');
    const progressBar = document.getElementById('progressBar');
    const progressText = document.getElementById('progressText');

    const progressPercentage = document.getElementById('progressPercentage');
    if (!epochsEl || !progressSection || !progressBar || !progressText) return;
    ...
    if (trainPlaceholder) trainPlaceholder.style.display = 'none';
